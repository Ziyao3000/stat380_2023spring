---
title: "Homework 5"
author: "[Yang Ziyao]{style='background-color: yellow;'}"
toc: true
...
In this assignment, we will explore `decision trees`,
`support vector machines` and
`neural networks for classification and regression`. The assignment is
designed to test your ability to fit and analyze these models with
different configurations and compare their performance.
...
# df$ocean_proximity <- as.integer(as.numeric(df$ocean_proximity))
```
...
lm_fit <- lm(median_house_value~.-households, df_train) # Insert your code here
...
-   The intercept represents the expected `median_house_value` when all
    other predictor variables are set to 0.

-   The estimated coefficients represent the expected change in the
    `median_house_value` for a one-unit increase in the corresponding
    predictor variable, holding all other variables constant.

-   The p-values suggest that all predictor variables are statistically
    significant, except for `ocean_proximityNEAR BAY`.

-   The R-squared value is 0.643, suggesting that this model explains
    64.3% of the variation in `median_house_value`. It indicates a
    moderate performance, but there could be room for improvement.
...
rmse_lm <- rmse(df_test$median_house_value,lm_predictions)  # Insert your code here
rmse_lm
```
...
```{R}
rpart_fit <- rpart(median_house_value~.-households, df_train) # Insert your code here
```
...
```{R}
svm_fit <- svm(median_house_value~.-households, df_train) # Insert your code here
...
nnet_fit <- NNet %>% 
  ...
  set_hparams(
    p=ncol(model.matrix(median_house_value~.-households, 
                        data = df_train)), q1=32, q2=16, q3=8 # Insert your code here
  ) %>%
  set_opt_hparams(
    lr=0.005 # Insert your code here
  ) %>%
  fit(
    data = list(
...
```{r}
nnet_predictions <- as_array(predict(nnet_fit, model.matrix(median_house_value~.-households, data = df_test)))
rmse_nnet <- rmse(df_test$median_house_value,nnet_predictions) # Insert your code here
rmse_nnet
```
...
-   Based on the results, the Support Vector Machine (SVM) performed the best among all the models. 
-   This could be due to SVM's ability to adapt well to the complexities of the underlying data.


```{r}
list(rmse_lm, rmse_rpart, rmse_svm, rmse_nnet) %>%
  set_names(c("Linear Regression", "Decision Tree", "Support Vector Machine", "Neural Network")) %>%
  as.data.frame() %>%
  t() %>%
...
overview <- function(pred_class, true_class) {
  ...
  true_positives <- sum(pred_class=="1" & true_class=="1") # Insert your code here
  true_negatives <- sum(pred_class=="0" & true_class=="0") # Insert your code here
  false_positives <- sum(pred_class=="1" & true_class=="0") # Insert your code here
  false_negatives <- sum(pred_class=="0" & true_class=="1") # Insert your code here
  true_positive_rate <- true_positives / (true_positives + false_negatives) # Insert your code here
  false_positive_rate <- false_positives / (false_positives + true_negatives)  # Insert your code here
  return(
    data.frame(
      accuracy = accuracy,
      error = error,
...
glm_classes <- glm_classes <- ifelse(predict(glm_fit, df_test, type = "response") > 0.5, 1, 0) # Insert your code here
```
...
```{r}
rpart_classes <- ifelse(predict(rpart_fit, df_test, type = "prob")[,2] > 0.5, 1, 0)
```
...
``` {r}
svm_fit <- svm(spam ~ ., data = df_train, kernel = "radial", type = "C-classification") # Insert your code here
```
...
``` {r}
svm_classes <- predict(svm_fit, df_test) # Insert your code here
```
...
nnet_fit <- NNet %>% 
  ...
  set_hparams(p=ncol(model.matrix(spam ~ ., data = df_train)), q1=32, q2=16, q3=8) %>%
  set_opt_hparams(lr=0.005) %>% 
  fit(
    data = list(
            model.matrix(spam ~ ., data = df_train),
            df_train %>% select(spam) %>% as.matrix
        ),
        valid_data = list(
            model.matrix(spam ~ ., data = df_test),
...
``` {r}
nnet_probs <- predict(nnet_fit, model.matrix(spam ~ ., data = df_test))
nnet_classes <- ifelse(nnet_probs > 0.5, 1, 0)
```
...
results <- bind_rows(
  glm_overview %>% mutate(model = "GLM"),
  rpart_overview %>% mutate(model = "Decision Tree"),
  svm_overview %>% mutate(model = "SVM"),
  nnet_overview %>% mutate(model = "Neural Network")
)
print(results)
```
...
* Based on the results, I would choose to use the GLM model. Although the neural network has better overall performance, it has a slightly higher false positive rate, which means it has a higher chance of classifying non-spam emails as spam. The cost of misclassifying non-spam emails as spam is high, so I would choose the GLM model.
...
``` {r}
grid <- seq(-10, 10, length.out = 100) # Insert your code here
df_test <- expand.grid(x1 = grid, x2 = grid) # Insert your code here
```
...
``` {r}
rpart_fit <- rpart(y~., df, method = "class") # Insert your code here
rpart_classes <- predict(rpart_fit, df_test, type = "class") # Insert your code here
```
...
```{r}
svm_fit <- svm(y~., df,type = "C-classification", kernel = "radial") # Insert your code here
svm_classes <- predict(svm_fit, df_test) # Insert your code here
...
fit_1 <- NN1 %>% 
  setup(
    loss = nn_cross_entropy_loss(), # Insert your code here
        optimizer = optim_adam, 
        metrics = list(
            luz_metric_accuracy()
        )
  ...
  set_hparams(
    p=2, q1=10, o = 3 # Insert your code here
  ) %>%
  set_opt_hparams(
    lr=0.005 # Insert your code here
  ) %>%
  fit(
    ...
    dataloader_options = list(batch_size = 15, shuffle = TRUE), # Insert your code here
    verbose = FALSE
  )
```
...
fit_0 <- NN0 %>% 
  setup(loss = nn_cross_entropy_loss(), # Insert your code here
        optimizer = optim_adam, 
        metrics = list(
            luz_metric_accuracy()
        )) %>%
...
fit_2 <- NN3 %>% 
  ...
  set_hparams(p=2, q1=10,q2=10,q3=10,o = 3) %>%
  set_opt_hparams(lr=0.005) %>%
  fit(data = list(
      df %>% select(x1, x2) %>% as.matrix,
      df$y %>% as.integer
...
* The difference of the models are their decision boundaries, meaning how they classify the data. 
* As the number of hidden layers increases the decision boundaries become more accurate and better at outlining the true boundaries between the different classes.
