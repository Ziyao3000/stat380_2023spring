...
[Link to the Github repository](https://github.com/Ziyao3000/hw2)
...
abalone <- as_tibble(read.csv(file = url,header = FALSE, col.names = abalone_col_names) )# Insert your code here
...
df <- drop_na(abalone) 
```
Non of the rows are dropped, which means there's no `NA`s in the dataset
...
df %>%
  ggplot(aes(x = sex, y = diameter, fill = sex))+
    geom_violin()
```
* Infant are generally smaller than females and males

* Males tend to have longer left tails
---
...
df%>%
  ...
  geom_point()
```
* There is an infant abalone who has a relative short length but larger diameter
---
...
```{R}
# Create a list of scatter plots using map2()
scatter_plots <- map2(.x = names(keep(df, is.numeric)), 
                      .y = rep("rings", length(names(keep(df, is.numeric)))), 
                      .f = ~ ggplot(df, aes(x = rings, y = .data[[.x]], color = sex)) +
                        labs(y = NULL, title = .x)+
                        geom_point())
# Arrange the plots in a grid using plot_grid() function from cowplot
...
summary(model)
```
* Intercept: height is -0.003803  when diameter is 0
* diameter: height increase 0.351376mm as diameter increase 1mm
* both are statistical significant for their p-values are all less than 0.05
---
...
```
* The linear model an appropriate fit for this relationship, because the dots are spread around our predicted line
...
# Insert your code here. 
predicted_height = coef(model)[1] + coef(model)[2] * new_diameters
plot(diameter, height)
